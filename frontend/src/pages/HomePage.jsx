  ...
  return (
    <div className="h-full overflow-y-auto bg-background">
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        {/* Welcome Banner */}
        <div className="bg-gradient-to-r from-[#219ebc] to-[#023047] rounded-2xl p-8 text-white">
          ...
          <div className="flex gap-4 flex-wrap">
            <Button className="bg-white text-[#023047] hover:bg-white/90" onClick={() => navigate('/friends')}>
        ...
        <div className="bg-card rounded-xl border border-border overflow-hidden">
          <div className="p-6 border-b border-border flex items-center justify-between">
            <h2 className="text-xl font-bold text-foreground">Parceiros Sugeridos</h2>
          ...
          {isLoading ? (
            ...
            <div className="p-6 grid gap-4">
              {users.map((partner) => {
                ...
                return (
                  <div key={partner.id} className="flex items-center gap-4 p-4 rounded-lg border border-border hover:bg-muted/50 transition-colors">
                    {/* Avatar */}
                    <div className="relative shrink-0">
                      {partner.profilePic ? (
                        <img src={partner.profilePic} alt={partner.fullName} className="w-16 h-16 rounded-full object-cover" />
                      ) : (
                        <div className="w-16 h-16 rounded-full bg-[#219ebc] flex items-center justify-center text-white text-xl font-bold">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <h3 className="font-semibold text-foreground">{partner.fullName}</h3>
                        {isOnline && <span className="text-xs text-[#10b981] font-medium">Online</span>}
                      </div>
                      <div className="flex items-center gap-2 text-sm text-muted-foreground mb-1">
                        <Globe className="w-3 h-3" />
                        <span>{partner.nativeLanguage} → {partner.learningLanguage}</span>
                      </div>
                      {partner.location && <p className="text-xs text-muted-foreground">{partner.location}</p>}
                      {partner.bio && <p className="text-sm text-muted-foreground mt-1 line-clamp-1">{partner.bio}</p>}
                    </div>
                    ...
                    <div className="flex gap-2 shrink-0">
                      <Button
                        size="sm"
                        className="bg-[#219ebc] hover:bg-[#023047] text-white"
